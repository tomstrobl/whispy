...
- Each test comes in two notebook forms (all in `examples/`, kept flat so relative paths to `../configs` and `demo_stimuli/` work): a minimal **building block** `building_block_<test>.ipynb` (config explanation + run + save, nothing else — the copy-paste source) and a finished **full experiment** `full_experiment_<test>.ipynb` (consent questionnaire → test → thank-you screen). Tests: `staircase_n_afc` (adaptive staircase driving N-AFC), `abx` (ABX discrimination, fixed trial list → percent correct), `drag_and_drop_mushra` (MUSHRA). `examples/building_block_consent.ipynb` runs the consent questionnaire that produces the anonymous `participant_id`. There is no standalone single-trial N-AFC notebook; the staircase notebooks are the N-AFC example (they drive `whispy.ui.NAFC`).
