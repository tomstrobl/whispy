...
- `configs/attributes.yml`: maps attribute name -> `{task, description, values, labels, neutral_value}` (rating scale definition shown by `DragAndDropMUSHRA` and used for the task text in `NAFC`). `NAFC` also honors a `task` set directly on the `screen` dict, which takes precedence over the `attributes.yml` lookup (lets a trial carry its prompt inline).
