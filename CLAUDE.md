...
- `configs/abx.yml`: a single self-contained config for `examples/abx.ipynb` (ABX). One file feeds `SoundDevice` (its `SoundDevice:` block), `ABX` (its `test:` = `shuffle_ab` / `ui:` blocks, passed as `abx_config`), and the notebook (`trial:` = prompt/metadata, `pairs:` = list of `{a, b, name}` comparisons, `experiment:` = `repetitions`/`shuffle_trials`/`seed`). Demo WAVs are generated by `examples/stimuli_abx/generate_abx_stimuli.py`.
